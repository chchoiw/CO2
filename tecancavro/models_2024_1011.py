...
class XCaliburD(Syringe):
    ...
    def extractToWaste(self, in_port, volume_ul, out_port=None,
        ...
        while not extracted:
            try:
                ...
                if ((self.sim_state['plunger_pos'] + steps) > 6000 & self.microstep in [0, "0"]) or \
                    ((self.sim_state['plunger_pos'] + steps) > 48000 & self.microstep in [1, "1"]) or retry:
                    self.logDebug('extractToWaste: move would exceed 6000/48000 '
    ...
    def _ulToSteps(self, volume_ul, microstep=None):
        ...
        if microstep in [0, "0", "Normal", "normal", "NORMAL"]:
            steps = volume_ul * (6000/self.syringe_ul)
        elif microstep in [1, "1", "Fine", "FINE", "fine"]:
